...
export function createMessageMethods(mongoose: typeof import('mongoose')): MessageMethods {
  ...
  async function updateMessageJapaneseAdvice(
    ...
    try {
      const Message = mongoose.models.Message as Model<IMessage>;
      /** Legacy rows may carry `metadata: null` (or omit the key). Seeding
       *  the parent object first keeps `$set: {'metadata.japaneseAdvice': …}`
       *  viable — the seed only matches rows that lack a usable metadata
       *  object, so normal rows skip it entirely. */
      const seeded = await Message.findOneAndUpdate(
        {
          messageId,
          user: userId,
          $or: [{ metadata: { $exists: false } }, { metadata: null }],
        },
        { $set: { metadata: {} } },
      );
      if (seeded === null && (await Message.exists({ messageId, user: userId })) === null) {
        throw new Error('Message not found or user not authorized.');
      }
